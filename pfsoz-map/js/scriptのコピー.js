...
L.tileLayer('tiles/{z}_{x}_{y}.gif', {
    ...
    maxNativeZoom: 4, //4
    maxZoom: 8, //+2
    attribution: 'Map created by <a href="https://www.pixiv.net/users/19606">arohaJ</a> for <a href="https://www.pixiv.net/artworks/101965643">"PFSOZ"</a> project.',
    //タイルの表示範囲指定（左上:右下）
    bounds: [
        [0, 0],
        [-190, 256]
...
L.imageOverlay('https://1686.site/pfsoz-map/tiles/0_0_0.gif',
//左上latlong、右下latlong
...
L.imageOverlay('https://1686.site/pfsoz-map/images/wz-rain.png',
//左上latlong、右下latlong
...
L.imageOverlay('https://1686.site/pfsoz-map/images/wz-snow.png',
L.latLngBounds([-8, 72], [-18, 92]),
{pane: 'whether-snow',
opacity:0.5}
).addTo(map);
map.createPane('whether-oasis');
L.imageOverlay('https://1686.site/pfsoz-map/images/wz-oasis.png',
...
var belongFilterButton = L.control.tagFilterButton({
    data: ['red', 'blue', 'purple', 'white', 'yellow', 'green', 'black', 'gray'],
    filterOnEveryClick: true,
    icon: '<i class="fas fa-flag"></i>',
    clearText: '所属：取り消す',
}).addTo(map);

var typeFilterButton = L.control.tagFilterButton({
    data: ['campground', 'store', 'utensils', 'seedling', 'dragon', 'image', 'exclamation-circle'],
    filterOnEveryClick: true,
...
// Home button ------------------------------------------------------------------------------------------------------
L.easyButton('fas fa-map', function(btn, easyMap){
...
function update_coord_references(map, marker, latLng) {
    ...
    var content = "<pre class=\"coords\">" +
        "latitude:" + digit_format(latLng.lat) + "\n" +
        "longitude:" + digit_format(latLng.lng) + "</pre>";
...
$.get('https://1686.site/pfsoz-map/marker.csv', function (csvString) {
    ...
    for (var i in data) {
        ...
        }).bindPopup('<h2 class="' + row.belongs + '">' + row.markertitle + '</h2><a class="pop-guildtags" href="https://www.pixiv.net/tags/' + row.markertag + '/artworks" target="_blank" rel="noopener noreferrer">' + row.markertag + '</a><a class="pop-pagelink" href="' + row.markerurl + '" target="_blank" rel="noopener noreferrer">Pixiv</a><p class="pop-tag-' + row.type + '"></p><a class="share-ban-twi" href="https://twitter.com/share?url=https://1686.site/pfsoz-map/%238/' + row.latitude + '/' + row.longitude + '&text=' + row.markertitle + '｜座標: %238/' + row.latitude + '/' + row.longitude + '" rel="nofollow noopener noreferrer" target="_blank"><i class="fab fa-twitter"></i> 位置を共有</a><div class="tbc-' + row.tbc + '"></div>');
        marker.addTo(map);
    }
});
...
var minimapAttrib = 'Map created by <a href="https://www.pixiv.net/users/19606">arohaJ</a> for <a href="https://www.pixiv.net/artworks/100516274">"PF"</a> project.';
var miniMapSetting = new L.TileLayer(minimapUrl, {minZoom: 0, maxZoom: 2, attribution: minimapAttrib,bounds: [[0, 0],[-190, 256]] });
